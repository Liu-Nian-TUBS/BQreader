...
def read_bq2(filepath):
    ...
    for content in bq:
        ...
        if read_status == 1:
            ...
            elif line[:18] == 'Communicating with':
                if 'ecus' not in bq_dic:
                    bq_dic['ecus'] = {}
                cur_ecu = line[-5:-2]
                if cur_ecu not in bq_dic['ecus']:
                    bq_dic['ecus'][cur_ecu] = {}
                    bq_dic['ecus'][cur_ecu]['mdx'] = 'unknown'
                read_status = 0
...
if __name__ == '__main__':
    print(read_bq2('BusQuery_2023-01-30_BECM.rtf'))